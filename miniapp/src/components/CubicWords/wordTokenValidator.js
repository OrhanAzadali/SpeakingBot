...
const VERIFIED_3_LETTER_ENGLISH = new Set([
...
// Verified 3-letter Russian vocabulary words (excluding particles and abbreviations)
const VERIFIED_3_LETTER_RUSSIAN = new Set([
...
// Verified 3-letter Spanish words
const VERIFIED_3_LETTER_SPANISH = new Set([
...
// Verified 3-letter German words
const VERIFIED_3_LETTER_GERMAN = new Set([
...
// Verified 3-letter French words
const VERIFIED_3_LETTER_FRENCH = new Set([
...
// Verified 3-letter Italian words
const VERIFIED_3_LETTER_ITALIAN = new Set([
...
// Blacklist of known Scrabble syllables, obscure abbreviations, or non-words
// that commonly appear when cubes land but are NOT authentic words.
const SCRABBLE_NOISE_BLACKLIST = new Set([
  ...
  // Russian syllables / particles / non-lemmas
  'ПРЕ', 'ПРИ', 'ПОД', 'НАД', 'РАЗ', 'ИЗО', 'ОТО', 'БЕЗ', 'МНР', 'МПС', 'НКО',
  'ЫЫЫ', 'ЩЩЩ', 'ЦЦЦ', 'ФФФ', 'ХХХ',
]);

/**
 * Phonotactic sanity check: ensures word contains acceptable vowel patterns
 * and does not consist of impossible consonant clusters.
 */
function hasValidPhonotactics(word, lang = 'english') {
  ...
  // Must contain at least one vowel (unless recognized Welsh loan like CWM, which is blacklisted anyway)
  if (!vowels.test(word)) {
    return false;
  }

  // Reject 4+ consecutive identical characters (e.g. "AAAA", "ZZZZ")
  if (/(.)\1\1\1/i.test(word)) {
    return false;
  }

  // In English / Romance languages, reject 4+ consecutive harsh consonants
  // (unless containing common blends like SPRINT, STRENGTH)
...
/**
 * Primary validator to determine if a candidate string is an authentic,
 * truly existing, recognized word token in the specified target language.
 */
export function isValidWordToken(rawWord, language = 'english') {
  ...
  // Blacklist check for all languages
