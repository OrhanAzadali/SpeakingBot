...
// Configurable free limits and Telegram Stars pricing
...
// Acknowledges Telegram webhook updates early (within 8s) to avoid double-replies
// during long voice/LLM pipeline runs
if (PUBLIC_URL) {
  app.use(
    WEBHOOK_PATH,
...
// Validates cryptographic HMAC hash of window.Telegram.WebApp.initData
...
// AI-powered Quiz grading: handles synonyms, explanations, and grammatical roles
...
async function generateVocabularyPdf(userId, language, outputPath) {
  ...
  return new Promise((resolve, reject) => {
    ...
    doc
      .fontSize(22)
      .font("Helvetica-Bold")
      .text("Personal Vocabulary Notebook", { align: "center" });

    doc
      .fontSize(11)
      .font("Helvetica")
      .text(`Language: ${language ? LANGUAGES[language] || language : "All"} | Generated: ${new Date().toLocaleDateString()}`, {
        align: "center",
      });
    ...
    const renderWordItem = (item, index) => {
      ...
      doc
        ...
        .font("Helvetica-Bold")
        .fillColor("#1e293b")
        .text(`${index + 1}. ${item.initial_form || item.word} `, { continued: true });
      ...
      if (item.part_of_speech) {
        doc
          ...
          .fillColor("#64748b")
          .text(`[${item.part_of_speech}]`, { continued: true });
      }
    ...
    if (data.active.length > 0) {
      doc
        ...
        .font("Helvetica-Bold")
        .fillColor("#4338ca")
        .text(`📚 Active Flashcards (${data.active.length})`);
      doc.moveDown(0.5);
    ...
    if (data.mastered.length > 0) {
      ...
      doc
        ...
        .font("Helvetica-Bold")
        .fillColor("#047857")
        .text(`🏆 Mastered Words (${data.mastered.length})`);
      doc.moveDown(0.5);
...
bot.command("start", async (ctx) => {
  ...
  await ctx.reply(
    ...
    "and automatically save base/infinitive words with context as smart flashcards.\n\n" +
...
bot.callbackQuery(/^level_(.+)$/, async (ctx) => {
  ...
  await ctx.editMessageText(
    ...
    `💬 Send a text or 🎙 voice message to start!\n` +
    `📥 Use /pdf to download your words as a PDF anytime.`,
    { parse_mode: "Markdown" }
  );
});
...
bot.on("message:voice", async (ctx) => {
  ...
  try {
    ...
    try {
      ...
      throw new Error(`Failed to download voice file: ${downloadErr.message}`);
    }
