...
export async function initDB() {
  await pool.query(`
    ...
    -- Subscription & usage-tracking fields (idempotent: no-ops if already present)
    ...
    -- Extended linguistic columns for base-form, grammatical role, synonyms, and context
    ...
    -- Kept separately from active flashcards so mastered words are preserved permanently.
  ...
  `);
  console.log("✅ Database tables & schema migrations ready");
}
...
// Extends (or starts) a user's premium window by `days` from whichever is later
...
// Checks and resets free-tier quota per day; premium users pass without counting
...
// Tracks total user messages across all time to trigger the periodic roadmap update every 5th message
...
// Ordered newest-first (DESC) so freshly added mistakes appear at the top of the deck immediately
...
// Spaced repetition update (SuperMemo-2 inspired)
...
// ── Quiz & Learned Words ──────────────────────────────────────────────────────
// 3 consecutive correct answers in Quiz mode permanently archives the card into learned_words
...
export async function getLearnedWords(userId, language) {
  const { rows } = await pool.query(
    "SELECT * FROM learned_words WHERE user_id = $1 AND language = $2 ORDER BY learned_at DESC",
    [userId, language]
  );
  return rows;
}
