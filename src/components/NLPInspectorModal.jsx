...
export const NLPInspectorModal = ({ token, onClose, targetLanguage = "English", onSaveToVocabulary }) => {
  ...
  const playAudio = () => {
    if ("speechSynthesis" in window) {
      ...
      utterance.lang = getLanguageSpeechCode(token.targetLanguage || targetLanguage);
      utterance.rate = 0.85;
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm animate-in fade-in">
      ...
      >
        ...
        <div className="p-6 space-y-5">
          <div className="flex items-center justify-between p-4 rounded-xl bg-slate-800/60 border border-slate-700/60">
            <div>
              <div className="flex items-center gap-3">
                ...
                {token.ipa && (
                  <span className="font-mono text-sm text-sky-400 bg-sky-950/50 px-2 py-0.5 rounded border border-sky-800/40">
                    {token.ipa}
                  </span>
                )}
              </div>
            ...
            <div className="flex items-center gap-2">
              <button
                onClick={playAudio}
                className="p-3 bg-sky-600 hover:bg-sky-500 text-white rounded-xl shadow-lg shadow-sky-600/20 transition-transform active:scale-95 cursor-pointer"
          ...
          <div className="p-3.5 rounded-xl bg-emerald-950/20 border border-emerald-500/20">
            ...
            <p className="text-sm font-semibold text-emerald-100">
              {token.mediatorTranslation || "Tərcümə mövcuddur"}
            </p>
          </div>

          {/* Morphology details */}
          {token.morphology && (
            <div className="p-3 rounded-xl bg-slate-800/30 border border-slate-800 text-xs text-slate-400">
        ...
        {/* Footer with Save to Vocabulary */}
        <div className="px-6 py-3.5 bg-slate-950/50 border-t border-slate-800 flex items-center justify-between">
          {onSaveToVocabulary ? (
            <SaveToVocabButton
              word={token.text || token.lemma}
              translation={token.mediatorTranslation || token.lemma}
              targetLanguage={token.targetLanguage || targetLanguage}
              pos={token.pos?.toLowerCase() || "noun"}
              ipa={token.ipa || ""}
          ...
          <button
            onClick={onClose}
            className="px-5 py-2 text-xs font-semibold rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-200 transition-colors cursor-pointer"
          >
            {t("close")}
          </button>
        </div>
