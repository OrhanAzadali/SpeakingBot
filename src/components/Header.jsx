...
export const Header = ({
  ...
  const handleCycleTheme = () => {
    // 1. If parent passed an onCycleTheme callback, use it
    if (typeof onCycleTheme === "function") {
      onCycleTheme();
      return;
    }
    // 2. Otherwise cycle internally via setId or setThemeId
    const currentId = themeId || activeTheme?.themeMeta?.id || "golden_ai";
    const currentIndex = PRESET_THEMES.findIndex((t) => t.id === currentId);
