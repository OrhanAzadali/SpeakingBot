// wordTokenValidator.js — Client-side tokenization and phonotactics validation

export const VERIFIED_3_LETTER_WORDS = {
  english: new Set([
...
export function hasValidPhonotactics(word, lang = 'english') {
  ...
  const isRussian = (lang || '').toLowerCase().includes('russ');
...
export function isValidWordToken(rawWord, language = 'english') {
  ...
  const verifiedSet = VERIFIED_3_LETTER_WORDS[langKey] || VERIFIED_3_LETTER_WORDS.english;
  if (cleanWord.length === 3) {
    if (verifiedSet.has(cleanWord)) {
      return { isValid: true, cleanWord, partOfSpeech: 'word' };
    }
    return { isValid: false, cleanWord, reason: 'Unverified 3-letter sequence' };
  }
