...
export const GrammarPDFPage = ({
  ...
  />
    ...
    /* Common Mistakes */
              ...
              {/* Target Key Vocabulary Lexicon */}
              {activeGuide.keyVocabulary && activeGuide.keyVocabulary.length > 0 && (
                <div>
                  <div className="flex items-center justify-between mb-3">
                    <h3 className="text-sm font-extrabold text-slate-100 flex items-center gap-2">
                      <Bookmark className="w-4 h-4 text-cyan-400" />
                      <span>Key Lexicon & Target Vocabulary ({activeGuide.keyVocabulary.length})</span>
                    </h3>
                    {onSaveToVocabulary && (
                      <button
                        type="button"
                        onClick={() => {
                          activeGuide.keyVocabulary.forEach((v) => {
                            onSaveToVocabulary({
                              word: v.word,
                      ...
                      >
                        + Bookmark All Lexicon
                      </button>
                    )}
                  </div>

                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
                    {activeGuide.keyVocabulary.map((v, vIdx) => (
                      <div
                        key={vIdx}
                      ...
                      >
                        <div className="min-w-0 flex-1">
                          <div className="flex items-center gap-2">
                            <span className="font-bold text-xs text-white">{v.word}</span>
                            {v.pos && (
                          ...
                          {v.example && (
                            <p className="text-[10px] text-slate-400 italic mt-1 bg-slate-900/60 p-1.5 rounded-lg border border-slate-800">
                              "{v.example}"
                            </p>
                          )}
                        </div>
