    ...
    if (currentIndex < questions.length - 1) {
      ...
      try {
        const res = await fetch("/api/user/level-test", {
          ...
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            userId: userProfile.userId,                       // ← FIX: pass userId
            targetLanguage: userProfile.targetLanguage,       // ← per-language level tracking
            level: assignedLevel,
            score: scorePercent,
            details: {
              correctCount,
        ...
        // ── Re-fetch the canonical profile from the server so the UI
        //    reflects what the server actually stored (not a local guess).
        try {
          ...
          if (profJson.success && profJson.data) {
            console.log("[Placement] server-confirmed profile:", {
              currentLevel: profJson.data.currentLevel,
              overallScore: profJson.data.overallScore,
              targetLanguage: profJson.data.targetLanguage,
            });
          }
        } catch (e) {
            ...
            <div className="p-8 rounded-3xl bg-slate-900 border border-slate-700/80 shadow-2xl text-center space-y-6 animate-in zoom-in-95">
              ...
              <div className="flex flex-col sm:flex-row items-center justify-center gap-3 pt-2">
                ...
                {onExitTest && (
                  <button
                    onClick={onExitTest}
                    className="w-full sm:w-auto flex items-center justify-center gap-2 px-5 py-2.5 rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-300 text-xs font-bold transition-colors"
                  >
                    <span>Exit Test</span>
                  </button>
                )}
