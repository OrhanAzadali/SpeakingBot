...
// Generous 50MB payload limit to safely handle large PDF files and book chapters
    ...
    Spanish: [
      {
        id: "vocab-es-1",
        word: "esperanza",
        translation: "ümid / надежда",
        targetLanguage: "Spanish",
        pos: "noun",
        ipa: "/es.peˈɾan.sa/",
        example: "Siempre mantenemos viva la esperanza.",
        savedAt: new Date().toISOString()
      }
    ]
  },
...
function loadUsersFromDisk() {
  try {
    ...
    console.warn("[Storage] Failed to read users.json:", err.message);
  }
  return { "default-user": JSON.parse(JSON.stringify(DEFAULT_USER_TEMPLATE)) };
}

let syncedUsersDatabase = loadUsersFromDisk();
...
function ensureUserVocabStructure(user) {
  if (!user.vocabularyByLanguage || typeof user.vocabularyByLanguage !== "object") {
    user.vocabularyByLanguage = JSON.parse(JSON.stringify(DEFAULT_USER_TEMPLATE.vocabularyByLanguage));
  }
  const activeLang = user.targetLanguage || "English";
  if (!Array.isArray(user.vocabularyByLanguage[activeLang])) {
...
function loadStoriesFromDisk() {
  try {
    ...
    console.warn("[Storage] Failed to read stories.json:", err.message);
  }
  return {};
}

let userCustomStories = loadStoriesFromDisk();

function saveStoriesToDisk() {
  try {
    fs.writeFileSync(STORIES_FILE, JSON.stringify(userCustomStories, null, 2));
  } catch (err) {
    console.error("[Storage] Failed to save stories.json:", err.message);
  }
}
...
// Active verified Gemini models for @google/genai: gemini-3.6-flash, gemini-3.5-flash-lite
  ...
  for (const model of candidateModels) {
    try {
      ...
      const timeoutPromise = new Promise((_, reject) => {
        setTimeout(() => reject(new Error("AI_REQUEST_TIMEOUT")), 25000);
      });
      ...
      console.warn(`[AI Engine] Model ${model} encountered notice: ${err.message?.slice(0, 100)}. Trying fallback...`);
    }
  }
...
// ==========================================
// ROBUST PDF EXTRACTION & SELECTION
// ==========================================
    ...
    if (PDFParse) {
      try {
        ...
        if (typeof parseFunc === "function") {
          const res = await parseFunc(buffer, { max: 35 });
          const raw = typeof res === "string" ? res : res?.text || "";
          const cleaned = cleanExtractedPdfText(raw);
          if (cleaned && cleaned.length > 50) {
            console.log(`[PDF Engine] Extracted ${cleaned.length} characters using pdf-parse.`);
            return cleaned;
          }
        }
      } catch (e1) {
        console.warn("[PDF Engine] pdf-parse call notice:", e1.message);
      }
    }
  } catch (err) {
    console.warn("[PDF Engine] Parser core notice:", err.message);
  }

  // Native regex text-stream fallback for unencrypted PDFs
  try {
    ...
    if (cleanChars.length > 100) {
      return cleanChars.slice(0, 35000);
    }
  } catch (e) {
    console.warn("[PDF Engine] Fallback extraction notice:", e.message);
  }

  return "";
}

function selectBestExcerpt(text, targetWords = 350) {
  if (!text) return "";
  ...
  if (paragraphs.length === 0) return text.slice(0, targetWords * 7);
    ...
    english: [
      {
        ...
        culturalLinguisticContext: "Wordsworth's lyrical celebration of language and nature exemplifies Romantic SVO prosody and emotive vocabulary.",
        paragraphs: [
        ...
        sentences: [
          {
            ...
            translation: "Bax ona, tarlada tək-tənha, o uzaqdakı dağlı qıza!",
            literaryNote: "Wordsworth employs the imperative 'Behold' to capture immediate sensory attention.",
            audioTime: "0:00 - 0:08"
          },
          {
        ...
        conversations: [
          {
            ...
            correctIndex: 0,
            botFeedback: "Superb! In Romantic poetics, preserving the pure acoustic sanctity of spontaneous expression is paramount."
          }
        ],
        exercises: [
          {
    ...
    german: [
      {
        ...
        culturalLinguisticContext: "Goethes philosophischer Monolog reflektiert den menschlichen Drang nach Wissen und die Grenzen der Wissenschaft.",
        paragraphs: [
        ...
        sentences: [
          {
            ...
            literaryNote: "Voranstellung des finiten Verbs verleiht dem Monolog dramatische Intensität.",
            audioTime: "0:00 - 0:08"
          }
        ],
        keyVocabulary: [
        ...
        exercises: [
          {
            ...
            correctIndex: 0,
            explanation: "'Der Tor' = der Narr / Unwissende; im Unterschied zu 'das Tor' = Pforte."
          }
        ]
      }
    ]
...
app.get("/api/user/profile", (req, res) => {
  ...
  if (!syncedUsersDatabase[userId]) {
    syncedUsersDatabase[userId] = JSON.parse(JSON.stringify(DEFAULT_USER_TEMPLATE));
...
// 3. User Vocabulary (GET, POST, DELETE)
app.get("/api/user/vocabulary", (req, res) => {
  ...
  if (!Array.isArray(user.vocabularyByLanguage[requestedTargetLang])) {
...
app.post("/api/user/vocabulary", (req, res) => {
  ...
  if (!Array.isArray(user.vocabularyByLanguage[lang])) {
  ...
  const entry = {
    ...
    word: cleanWord,
    translation: translation || meaning || cleanWord,
    pos: pos || "noun",
    ipa: ipa || "",
    example: example || "",
    targetLanguage: lang,
  ...
  res.json({
    success: true,
    message: `Added "${cleanWord}" to ${lang} vocabulary.`,
    data: list,
    allVocabularies: user.vocabularyByLanguage,
    countsByLanguage
  });
...
// 4. Target & Mediator Language Sync
app.post("/api/user/target-language", (req, res) => {
  ...
  if (!Array.isArray(user.vocabularyByLanguage[targetLanguage])) {
    user.vocabularyByLanguage[targetLanguage] = [];
  }
  user.savedVocabulary = user.vocabularyByLanguage[targetLanguage];
  user.lastSyncedAt = new Date().toISOString();
  saveUsersToDisk();
  ...
  res.json({
    success: true,
    message: `Target language switched to ${targetLanguage}.`,
    data: user,
    targetLanguage,
...
app.post("/api/user/mediator-language", (req, res) => {
  ...
  syncedUsersDatabase[userId].lastSyncedAt = new Date().toISOString();
  saveUsersToDisk();
...
app.post("/api/bot/sync", (req, res) => {
  ...
  if (botUpdate) {
    ...
    if (botUpdate.targetLanguage) {
      ...
      if (!Array.isArray(user.vocabularyByLanguage[user.targetLanguage])) {
...
// 6. Custom Stories & PDF Upload
app.get("/api/stories/custom-list", (req, res) => {
  const userId = String(req.query.userId || "default-user");
  const targetLanguage = String(req.query.targetLanguage || "English");
  const userStories = (userCustomStories[userId] || []).filter(
    (s) => (s.targetLanguage || "English").toLowerCase() === targetLanguage.toLowerCase()
  );
...
app.post("/api/stories/upload-pdf-book", async (req, res) => {
  try {
    const {
      userId = "default-user",
      fileBase64,
      pdfData, // accept both field names
      fileText = "",
      fileName = "Book_Excerpt.pdf",
    ...
    const rawBase64 = fileBase64 || pdfData || "";
    let extractedText = String(fileText || "").trim();

    if (!extractedText && rawBase64) {
      try {
        const cleanBase64 = rawBase64.replace(/^data:[^;]+;base64,/, "");
        const buffer = Buffer.from(cleanBase64, "base64");
        extractedText = await extractTextFromPdfBuffer(buffer);
      } catch (err) {
        console.warn("[PDF Upload] Base64 buffer parse notice:", err.message);
      }
    }

    let excerptSlice = "";
    if (extractedText && extractedText.length > 50) {
      excerptSlice = selectBestExcerpt(extractedText, 350);
    } else {
      excerptSlice = `An authentic literary chapter passage from "${bookTitle}" by ${author} in ${targetLanguage}.`;
    }
...
Mediator Language for translations & explanations: ${mediatorLanguage} (az: Azerbaijani, ru: Russian, tr: Turkish, es: Spanish, en: English, de: German)

Here is the authentic text excerpt from the book:
"""
${excerptSlice}
"""

Synthesize a complete, interactive Classic Story reading and audio study module based strictly on this book and excerpt.
Return ONLY valid JSON matching this exact schema without extra keys:
{
  "title": "${bookTitle}",
  "author": "${author}",
  "authorEra": "Classic Literature",
  "level": "${userLevel}",
  "mode": "both",
  "duration": "4 min read • 2 min audio",
  ...
  "paragraphs": [
    "Paragraph 1 text from the excerpt in ${targetLanguage}",
    "Paragraph 2 text from the excerpt in ${targetLanguage}"
  ],
  "sentences": [
    {
      "text": "Sentence in ${targetLanguage}",
      "translation": "Translation in ${mediatorLanguage}",
      "literaryNote": "Pedagogical commentary on syntax or rhetoric",
      "audioTime": "0:00 - 0:08"
    }
  ],
  "keyVocabulary": [
    {
      ...
      "translation": "translation in ${mediatorLanguage}",
      "cefr": "${userLevel}",
      "example": "Contextual sentence in ${targetLanguage}"
    }
  ],
  "stylisticDevices": [
    {
      "device": "Literary Device Name",
      "exampleFromText": "quote from text",
      "explanation": "Explanation in English or ${mediatorLanguage}"
    }
  ],
  "conversations": [
    {
      "id": "socratic-1",
      "persona": "SpeakBot Socratic Mentor",
      "prompt": "Deep Socratic question testing literary comprehension and psychological perspective of this excerpt",
      "options": [
        "Thoughtful, text-grounded interpretation",
        "Plausible but erroneous distractor",
        "Superficial incorrect reading"
      ],
      "correctIndex": 0,
      "botFeedback": "Detailed pedagogical feedback validating the insight."
    }
  ],
  "exercises": [
    {
      "id": "task-1",
      "question": "Comprehension or grammar question regarding this excerpt",
      "options": ["Option A", "Option B", "Option C"],
      "correctIndex": 0,
      "explanation": "Explanation directly referencing the text."
    }
  ]
}`;

    let parsedStory = null;
    const rawAi = await callGeminiWithResilience(aiPrompt);
    if (rawAi) {
      try {
        const clean = rawAi.replace(/```json\n?|\n?```/g, "").trim();
        parsedStory = JSON.parse(clean);
      } catch (parseErr) {
        console.warn("[PDF Upload] JSON parse warning:", parseErr.message);
      }
    }

    if (!parsedStory || !parsedStory.sentences || parsedStory.sentences.length === 0) {
      // Clean fallback
      const sentences = excerptSlice.match(/[^.!?]+[.!?]+/g) || [excerptSlice];
      parsedStory = {
        title: bookTitle,
        author: author,
        authorEra: "Classic Literature",
        ...
        duration: "4 min read • 2 min audio",
        targetLanguage,
        culturalLinguisticContext: `An excerpt from "${bookTitle}" by ${author} processed for ${targetLanguage} language acquisition.`,
        paragraphs: [excerptSlice],
        sentences: sentences.slice(0, 4).map((s, idx) => ({
          text: s.trim(),
          translation: `[${mediatorLanguage.toUpperCase()}] ${s.trim()}`,
          literaryNote: `Syntactic flow analyzed for ${targetLanguage} learners at ${userLevel} level.`,
        ...
        keyVocabulary: [
          {
            ...
            ipa: "/rɪˈflekʃn/",
            pos: "noun",
            translation: "mülahizə, düşüncə",
            cefr: userLevel,
            example: `Key linguistic reflection in ${bookTitle}.`
          }
        ],
        stylisticDevices: [
          {
            device: "Narrative Prose",
            exampleFromText: sentences[0]?.slice(0, 40) || bookTitle,
            explanation: `Authentic ${targetLanguage} syntax and discourse flow.`
          }
        ],
        conversations: [
          {
            id: "socratic-fallback-1",
            persona: "SpeakBot Socratic Mentor",
            prompt: `What is the central perspective or tone conveyed in this excerpt from "${bookTitle}"?`,
            options: [
              "Reflective narrative exposition and thematic observation.",
              "A casual phone conversation.",
              "A mathematical reference manual."
            ],
            correctIndex: 0,
        ...
        exercises: [
          {
            id: "task-fallback-1",
            question: `What is the primary subject matter of this excerpt from "${bookTitle}"?`,
            options: [bookTitle, "Unrelated topic", "Mechanical computation"],
            correctIndex: 0,
            explanation: "Directly verified against the uploaded book text."
          }
        ]
      };
    }
    ...
    if (!Array.isArray(userCustomStories[userId])) {
    ...
    res.json({
      success: true,
      message: `Successfully processed "${bookTitle}".`,
      story: finalStory
    });
  } catch (error) {
    ...
    res.status(500).json({
      success: false,
      error: error.message || "Failed to process PDF book."
    });
  }
});
...
app.post("/api/socratic/chat", async (req, res) => {
  try {
    ...
    const prompt = `You are the SpeakBot Socratic Literary Mentor discussing "${bookTitle}" by ${author}.
Target Language: ${targetLanguage}.
Mediator Language for translation notes: ${mediatorLanguage}.
The passage:
"""${excerpt.slice(0, 1000)}"""

Learner said: "${userMessage}".
Provide an encouraging Socratic response in ${targetLanguage} that guides their literary and linguistic insight. Return JSON:
{
  "reply": "Your Socratic conversational response in ${targetLanguage}..."
}`;

    const raw = await callGeminiWithResilience(prompt);
    let reply = `That is a thoughtful perspective on "${bookTitle}". In this passage, ${author} uses nuanced sentence structure to mirror the narrator's emotional journey. How does this phrasing influence your perception of the protagonist?`;
...
// 8. Dynamic AI Word Generation for Games
const GAME_FALLBACK_WORDS = {
  English: ["harmony", "solitary", "journey", "whisper", "resilience", "eloquence", "courage", "explore"],
  German: ["Freundschaft", "Reise", "Hoffnung", "Verwandlung", "Begeisterung", "Scharfsinn", "Geduld"],
  Spanish: ["esperanza", "amistad", "desafío", "resiliencia", "efímero", "perspicacia", "alegría"],
  French: ["voyage", "espoir", "défi", "éloquence", "éphémère", "apprivoiser", "sagesse"],
  Italian: ["viaggio", "speranza", "sfida", "resilienza", "perspicace", "amicizia", "armonia"],
  Russian: ["путешествие", "надежда", "испытание", "стойкость", "проницательность", "дружба"],
  Turkish: ["yolculuk", "umut", "meydan okuma", "feraset", "arkadaşlık", "sabır", "keşfetmek"]
};

app.post("/api/games/generate-words", async (req, res) => {
  try {
    const { targetLanguage = "English", userLevel = "B1", count = 6 } = req.body;
    const prompt = `Generate exactly ${count} distinctive, authentic ${targetLanguage} vocabulary words suitable for CEFR ${userLevel} level. Return ONLY a JSON array of strings: ["word1", "word2", ...]`;
    const raw = await callGeminiWithResilience(prompt);
    if (raw) {
      try {
        const clean = raw.replace(/```json\n?|\n?```/g, "").trim();
        ...
        if (Array.isArray(parsed) && parsed.length > 0) {
          return res.json({ success: true, words: parsed.slice(0, count) });
        }
      } catch (e) {
        console.warn("Games generate-words parse notice:", e.message);
      }
    }
  } catch (err) {
    console.warn("Games generate-words notice:", err.message);
  }

  const list = GAME_FALLBACK_WORDS[targetLanguage] || GAME_FALLBACK_WORDS.English;
  res.json({ success: true, words: list.slice(0, 6) });
});

app.post("/api/games/generate-vocabulary", async (req, res) => {
  try {
    const { targetLanguage = "English", mediatorLanguage = "az", userLevel = "B1", count = 6 } = req.body;
    const prompt = `Generate exactly ${count} vocabulary items for CEFR ${userLevel} in ${targetLanguage}.
Provide accurate translation in ${mediatorLanguage}. Return ONLY valid JSON array:
[
  {
    "word": "term in ${targetLanguage}",
    "translation": "meaning in ${mediatorLanguage}",
    "ipa": "/phonetic/",
    "pos": "noun",
    "level": "${userLevel}",
    ...
    if (raw) {
      try {
        ...
        const parsed = JSON.parse(clean);
        if (Array.isArray(parsed) && parsed.length > 0) {
          return res.json({ success: true, vocabulary: parsed });
        }
      } catch (e) {
        console.warn("Games generate-vocabulary parse notice:", e.message);
      }
    }
  } catch (err) {
    console.warn("Games generate-vocabulary notice:", err.message);
  }

  const fallbackWords = GAME_FALLBACK_WORDS[targetLanguage] || GAME_FALLBACK_WORDS.English;
  const vocab = fallbackWords.slice(0, count).map((w, idx) => ({
    id: `fb-${idx}`,
    word: w,
    ...
    pos: "noun",
    level: userLevel,
    example: `A key vocabulary word in ${targetLanguage}: ${w}.`
  }));
  res.json({ success: true, vocabulary: vocab });
});

// 9. AI Generator: Roadmaps & Grammar Guides
app.post("/api/gemini/generate-roadmap", async (req, res) => {
  try {
    const { topic, level = "B1", targetLanguage = "English", mediatorLanguage = "az" } = req.body;
    const prompt = `You are SpeakBot's Chief Curriculum Architect.
Generate a structured, highly informative learning Roadmap for "${topic}" in ${targetLanguage} at CEFR Level ${level}.
Mediator Language for explanations and translations: ${mediatorLanguage}.

Return strictly valid JSON:
{
  "title": "Roadmap: ${topic} (${level})",
  "category": "Grammar",
  "level": "${level}",
  "targetLanguage": "${targetLanguage}",
  "estimatedDuration": "3 Weeks",
  "summary": "Detailed, pedagogical overview of how this roadmap builds proficiency in ${topic} for ${targetLanguage}.",
  "milestones": [
    {
      "step": 1,
      "title": "Foundation & Core Principles",
      "description": "Exhaustive learning objective and grammar focus.",
      "grammarPoint": "Specific syntactic formula or rule",
      "sampleSentence": "Example sentence in ${targetLanguage}",
      "tokens": [
        {
          "text": "sample",
          "lemma": "sample",
          ...
          "cefrLevel": "${level}",
          "ipa": "/sæmpl/",
          "mediatorTranslation": "nümunə"
        }
      ]
    },
    {
      "step": 2,
      "title": "Application & Practical Nuances",
      "description": "Extending to complex sentence clauses and stylistic balance.",
      "grammarPoint": "Secondary rule and modal interactions",
      "sampleSentence": "Second example sentence in ${targetLanguage}",
      "tokens": []
    },
    {
      "step": 3,
      "title": "Advanced Mastery & Error Prevention",
      "description": "Remediating common pitfalls and native language interference.",
      "grammarPoint": "Advanced inversion or conditional nuance",
      "sampleSentence": "Third example sentence in ${targetLanguage}",
      "tokens": []
    }
  ],
  "checkpointQuestions": [
    {
      "question": "Diagnostic checkpoint question regarding ${topic}?",
      "options": ["Option A (Correct)", "Option B", "Option C", "Option D"],
      "correctIndex": 0,
      "explanation": "Detailed explanation in ${mediatorLanguage}."
    }
  ]
}`;
    ...
    if (raw) {
      try {
        ...
        console.warn("Roadmap parse notice:", e.message);
      }
    }

    // Fallback
    const fallbackRoadmap = {
      title: `Curriculum Roadmap: ${topic} (${level})`,
      category: "Grammar",
      level,
      targetLanguage,
      estimatedDuration: "3 Weeks",
      summary: `Structured step-by-step pathway for mastering ${topic} in ${targetLanguage}.`,
      milestones: [
        {
          step: 1,
          title: "Syntactic Fundamentals",
          description: `Master basic sentence construction and grammar paradigms for ${topic}.`,
          grammarPoint: "Core Constituent Alignment",
          sampleSentence: `Learning ${topic} strengthens communication.`,
          tokens: []
        },
        {
          step: 2,
          title: "Intermediate Application",
          description: "Apply structures in varied conversational and written contexts.",
          grammarPoint: "Aspect & Clause Integration",
          sampleSentence: `Diligent practice yields measurable fluency.`,
          tokens: []
        }
      ],
      checkpointQuestions: [
        {
          ...
          options: ["Consistent application and pattern recognition", "Memorizing without context", "Skipping fundamentals"],
          correctIndex: 0,
          explanation: "Consistent contextual practice solidifies grammatical structures."
        }
      ]
    };
    res.json({ success: true, roadmap: fallbackRoadmap });
  } catch (err) {
...
app.post("/api/gemini/generate-grammar-guide", async (req, res) => {
  try {
    const { title, level = "B1", category = "Grammar", targetLanguage = "English", mediatorLanguage = "az" } = req.body;
    const prompt = `You are SpeakBot's Chief Linguist. Create an exhaustive Grammar Study Guide for "${title}" in ${targetLanguage} (CEFR ${level}).
...
Return strictly valid JSON:
{
  ...
  "targetLanguage": "${targetLanguage}",
  "pagesCount": 5,
  "summary": "Comprehensive academic overview of ${title} in ${targetLanguage}.",
  "coreRules": [
    {
      "ruleTitle": "Core Principle 1",
      "explanationInMediator": "Clear explanation in ${mediatorLanguage} with high grammatical accuracy.",
      "formula": "Subject + Verb + Object",
      "example": "Sample sentence in ${targetLanguage}",
      "tokens": [
        {
          "text": "Sample",
          "lemma": "sample",
    ...
    {
      "ruleTitle": "Core Principle 2",
      "explanationInMediator": "Secondary rule explanation in ${mediatorLanguage}.",
      "formula": "Auxiliary + Subject + Verb",
      "example": "Second sample sentence in ${targetLanguage}",
      "tokens": []
    }
  ],
  "commonMistakes": [
    {
      "incorrect": "Incorrect usage example in ${targetLanguage}",
      "correct": "Correct usage in ${targetLanguage}",
      "reason": "Clear explanation in ${mediatorLanguage} of why this mistake occurs."
    }
  ],
  "practiceExercises": [
    {
      "instruction": "Select the grammatically sound option:",
      "question": "Practice question in context?",
      "options": ["Option A (Correct)", "Option B", "Option C", "Option D"],
      "correctIndex": 0,
      "explanation": "Detailed explanation in ${mediatorLanguage}."
    }
  ...
  "keyVocabulary": [
    {
      ...
      "ipa": "/ruːl/",
      "translation": "qayda",
      "example": "Always follow this grammar rule."
    }
  ]
}`;
    ...
    if (raw) {
      try {
        ...
        console.warn("Grammar guide parse notice:", e.message);
      }
    }
    ...
    const fallbackGuide = {
      ...
      targetLanguage,
      pagesCount: 4,
      summary: `Essential reference guide covering the structural rules and common pitfalls of ${title}.`,
      coreRules: [
        {
          ruleTitle: "Constituent Concord",
          explanationInMediator: `Cümlə üzvlərinin qrammatik uzlaşması qaydaları (${mediatorLanguage.toUpperCase()}).`,
          formula: "Subject + Predicate + Complement",
          example: `This is a fundamental rule in ${targetLanguage}.`,
          tokens: []
        }
      ],
      commonMistakes: [
        {
          incorrect: "Incorrect structure pattern",
          correct: "Standard grammatical form",
          reason: "Respect correct word order and agreement."
        }
      ],
      practiceExercises: [
        {
          instruction: "Select the correct form:",
          question: `Which option demonstrates accurate usage in ${targetLanguage}?`,
          options: ["Correct standard sentence", "Malformed option", "Fragment"],
          correctIndex: 0,
          explanation: "Adheres to standard grammatical agreement."
        }
      ],
      keyVocabulary: []
    };
...
// 10. Story Progress & Skill Tests
app.post("/api/stories/progress", (req, res) => {
  const { userId = "default-user", score = 85, scoreDelta = 4, mode = "reading" } = req.body;
  if (!syncedUsersDatabase[userId]) {
    syncedUsersDatabase[userId] = JSON.parse(JSON.stringify(DEFAULT_USER_TEMPLATE));
    syncedUsersDatabase[userId].userId = userId;
  }
  ...
  const current = user.skillScores?.[skillKey] ?? 70;
  const updated = Math.min(100, Math.max(20, current + scoreDelta));
  user.skillScores[skillKey] = updated;
  user.lastSyncedAt = new Date().toISOString();
  saveUsersToDisk();

  res.json({
    success: true,
    message: `Progress recorded. ${skillKey} updated to ${updated}%.`,
    skillScores: user.skillScores
  });
});

app.post("/api/user/skill-test", (req, res) => {
  const { userId = "default-user", skill = "grammar", scoreDelta = 5, score } = req.body;
  if (!syncedUsersDatabase[userId]) {
    syncedUsersDatabase[userId] = JSON.parse(JSON.stringify(DEFAULT_USER_TEMPLATE));
    syncedUsersDatabase[userId].userId = userId;
  }
  const user = syncedUsersDatabase[userId];
  if (!user.skillScores) user.skillScores = {};

  const newScore = score !== undefined ? score : Math.min(100, Math.max(20, (user.skillScores[skill] ?? 70) + scoreDelta));
  user.skillScores[skill] = newScore;
  user.lastSyncedAt = new Date().toISOString();
  saveUsersToDisk();
...
// ==========================================
// VITE SPA DEVELOPMENT & PRODUCTION SETUP
// ==========================================
